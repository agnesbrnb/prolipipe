...
def parser() : 
    ...
    parser.add_argument("-nr", "--no-report", dest="no_report", action="store_false", help="Use if you don't want interactive Quarto report.")
...
def main():
    ...
    if options.no_report:
        quarto_command = f"""
        quarto render report.qmd --output-dir quarto_report -P main_dir:{os.path.dirname(output_metabo)} -P input_dir_name:{os.path.basename(output_metabo)} &&
        rm -rf {os.path.dirname(output_metabo)}/quarto_report
        mv quarto_report {os.path.dirname(output_metabo)}
        """
